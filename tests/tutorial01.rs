...
use rsmpeg::{
    avcodec::{AVCodec, AVCodecContext, AVPicture},
...
fn _main(file: &CStr, out_folder: &str) -> Result<()> {
    ...
    let num_bytes = AVPicture::get_size(
        ffi::AVPixelFormat_AV_PIX_FMT_RGB24,
        decode_context.width,
        decode_context.height,
    )
    .unwrap();
    let buffer = vec![0u8; num_bytes as usize];

    let mut frame_rgb = AVFrame::new();
    unsafe {
        frame_rgb
            .fill_arrays(
                buffer.as_ptr(),
...
fn _main_test() {
    _main(
        cstr!("tests/utils/tutorial01/centaur.mpg"),
        "tests/utils/tutorial01/decoded/",
    )
    .unwrap();
}
